...
To begin, I had to obtain the game data for both the regular season and the section tournaments. [MN Hockey Hub](http://www.mnhockeyhub.com/) is a website that has the results for all high school hockey games played in Minnesota. I used the **rvest** package to scrape these data. 

```{r eval=FALSE}
# Beginning and ending dates for games
start = as.Date("2017/11/22", format = "%Y/%m/%d")
end   = as.Date("2018/03/02",format="%Y/%m/%d")

# Create an empty list with 101 elements
results = rep(list(NA), 101) 
...
library(dplyr)
hockey = readr::read_csv("~/Dropbox/data/mn-hockey-boys-2018-03-03.csv")
hockey
```
...
Comparing these to the actual tournament seeds, we see some differences in the top five seeds. In Class A, the coaches included Thief River Falls in the top five, while our Elo model put Monticello in the top five. There are also some ranking differences between the coaches picks and our Elo model for the other teams. 

In Class AA, the Elo model and the coaches selected the same top five teams, but had a different rank ordering for the bottom three of those five. Both ranked Minnetonka \#1 and Edina \#2.

## Simulate the State Tournament

We can now use these Elo ratings to determine the probability that one team would beat another. For example, in the Class A quarterfinal game between Hermantown (\#1) and Monticello (\#8), Hermantown's probability of beating Monticello is 0.547 .

```{r}
predict(elo_reg_season, data.frame(home = "Hermantown", visitor = "Monticello"))
```

I simulated the state tournament by using a random-number generator to determine the winner of each game. For instance, to simulate the Hermantown/Monticello game, I used the `runif()` function to generate a random number drawn from the uniform distribution between 0 and 1. If the result is less than or equal to 0.547, Hermantown is the winner; if not, Monticello wins. The syntax for simulating the Class A state tournament 10,000 times is below.

```{r cache=TRUE}
# Enter teams in bracket order (not by ranking)
team_1 = "Hermantown"
team_2 = "Monticello"
team_3 = "Alexandria"
team_4 = "Thief River Falls"
team_5 = "Orono"
...
Based on these simulations, Mahtomedi has about a 30\% chance of winning the Class A tournament.
...
team_1 = "Minnetonka"
team_2 = "Hill-Murray"
team_3 = "St. Thomas Academy"
team_4 = "Centennial"
team_5 = "Duluth East"
team_6 = "STMA"
team_7 = "Edina"
team_8 = "Lakeville North"
...
The simulation results suggest that both Minnetonka and Edina have a pretty good shot of winning the tournament (slight edge to Minnetonka). 
